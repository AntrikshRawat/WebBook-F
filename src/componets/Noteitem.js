...
export default function Noteitem(props) {
  ...
  return (
        <>
          <div className="container bg-dark-subtle mb-4 noteitem">
          <span className="badge text-bg-info m-2">{tag}</span>
            <h3 className='mt-3 text-center title'>{title}</h3>
            {tag==="link"?<a target='_main' href={description} className='des'>{description}</a>:<p className='des'>{description}</p>}
